...
## 2026-08-09 — first real init-project run (a directus+nextjs project)
...
a live project the same day: `M15-T7` sat as `backlog` next to twelve runnable
...
## 2026-08-27 — client-package, one session on a live project: six defects, and five of them were silent
...
rows in that project's ledger and 0 `comment` rows. Everything about pre-filling a previous
...
пути, вписанному в код: `/Users/<имя>/…/<клиентский-проект>/macstack/client`. Имя
...
нажал: поле просто остаётся пустым. И в журнале проекта это выглядело как «клиент не
...
## 2026-08-28 — `render.py` считал покрытие по чужим веткам, а `--help` создавал каталог

**Проблема, первая.** `scan_tests` обходил дерево проекта, исключая только
`node_modules`, `.git`, `.next` и `dist`. На живом проекте в `.claude/worktrees/`
лежали четыре worktree'а агентов — ПОЛНЫЕ чекауты того же репозитория. Сканер обошёл
их наравне с рабочим деревом, и в `generated/TEST-CASES.md` уехали строки вида

    - `X-01` Task feed … — `.claude/worktrees/agent-a52f8dda…/tests/e2e/panels.e2e.spec.ts`

Некрасивый путь — меньшая половина беды. Большая: кейс мог оказаться «проверенным»
тестом с ветки, которая не слита и, возможно, не будет. `TEST-CASES.md` читает
заказчик как отчёт о готовности.

**Проблема, вторая.** `main()` разбирал аргументы вручную и складывал всё
непонятное в `positional`, откуда брался корень проекта. `render.py --help` создавал
каталог с именем `--help` и писал в него пять документов. Это происходило дважды за
одну сессию, второй раз — уже зная про первый.

**Починка.** `SKIP_DIRS` расширен (`.claude`, `.codex`, `build`, `coverage`, `.turbo`,
`.venv`, `__pycache__`), и рядом — проверка на вложенный `.git`: она ловит чекаут под
любым именем, а не только под знакомым. В `main()` добавлены `USAGE`, обработка
`-h/--help` и отказ на любой неизвестный флаг вместо молчаливого превращения его в путь.

**Тест.** `tests/test_render_scan.py`, пять случаев. Три про то, что не считается
(`.claude`, `.codex`, чекаут под обычным именем), и — обязательно — два про то, что
считается: рабочее дерево и обычный workspace-пакет. Без последних двух первые три
зелены на сканере, который не считает ничего. Доказано мутацией: возврат старого
списка исключений красит ровно 3 из 5.

**Корневая причина.** Обе — одна и та же: вход не проверялся, а угадывался. Список
каталогов перечислял то, что помнили на день написания, и ничего не говорил про класс
«вложенный чекаут»; разбор аргументов принимал за путь всё, что не узнал. Список
устаревает молча, отказ — нет.

**Тяжесть:** Major. Ложное покрытие в документе, который заказчик читает как отчёт о
готовности, — это не косметика: по нему принимают решение, что можно не проверять.

## 2026-08-28 — префикс id ничего не говорил о файле, а буква `T` значила две разные вещи

**Проблема.** Владелец проекта прочитал отчёт, где я ссылался на `A29`, `M15-T11`
и `C-14`, и не смог понять ни что это за записи, ни в каком файле их искать. Его
разбор был точным:

- `A27` — вопрос, но файл называется `OPEN-QUESTIONS.md`. Откуда `A`?
- `M15-T11` — задача, но файл `TASKS.md`. Откуда `M`?
- `T-19` — кейс тренинг-центра, а `T` в `M15-T11` — задача. **Одна буква, два
  пространства имён.** По id нельзя было сказать, что перед тобой.

Он же предложил решение: `CC`, `CT`, `CO` — буква вида плюс буква роли.

**Починка.** Префикс теперь называет файл: `C` = case → `USER-CASES.md`,
`Q` = question → `OPEN-QUESTIONS.md`. Вторая буква сохраняет прежний смысл.
`M15-T11` не менялся: `M` называет ВЕХУ, `T` — задачу внутри неё, обе живут в
`TASKS.md`, и форма выбрана под соглашение о коммитах. Это не исключение, а
неверно прочитанный id — и теперь так и написано в `SKILL.md`.

Паттерны сделаны **обратно совместимыми** (`^C?[A-Z]-[0-9]{2}$`,
`^Q?[AB][0-9]+$`): смена без совместимости уронила бы линт у каждого заведённого
проекта разом. Проверено — живой проект на старой форме даёт те же 67 находок,
что и до правки.

**Скрипт миграции** — `migrate_ids.py`, и в нём два измеренных урока:

1. **Слепой regex нельзя.** `^[A-Z]-\d{2}$` совпадает и с id ЧУЖОГО документа: на
   живом проекте по той же форме записано немецкое ТЗ заказчика (`E-05`, `A-08`, `NF-07`).
   Скрипт читает ОБЪЯВЛЕНИЯ — заголовки в двух клиентских документах — и
   переименовывает ровно их.
2. **Неизменяемое лежит внутри `macstack/`.** Первая версия сравнивала путь с
   началом строки, и `inbox/` с `history/handoffs/` не отсекались ни разу.
   Разница измерена: **9041 замена против 2964** — две трети всех совпадений
   сидели в пакетах, уже отправленных заказчику, где id это то, что он прочитал.

**Тест** — `tests/test_migrate_ids.py`, шесть случаев, включая два контрольных
(чужой id остаётся, отправленный пакет остаётся). Доказан мутацией: снятие
защиты неизменяемого красит именно тот тест, который про неё. Первая мутация
НЕ покрасила ничего — защита оказалась двойной, и это выяснилось только потому,
что мутацию прогнали, а не предположили.

**Корневая причина.** Схема имён проектировалась под программу, которая по ним
грепает, и ни разу не проверялась на человеке, который по ним ищет файл. Оба
дефекта — `A` вместо `Q` и `T` в двух смыслах — видны с первого взгляда тому, кто
читает, и невидимы тому, кто пишет регулярку.

**Тяжесть:** Major. Документами пользуется владелец проекта, а не только плагин.

### Дополнение того же дня: миграцию доломали четыре пропущенных регулярки

Перевод живого проекта на новую форму нашёл то, чего не нашёл первый обход по
файлам. **Линт после миграции дал 7 находок вместо 67 — и это была не победа, а
регрессия.** Правило `12.11` («каждое обещание проверено») читает
`generated/TEST-CASES.md` регуляркой `^-\s+`([A-Z]-\d{2})`…` и с `CC-14`
перестало совпадать вообще: шестьдесят непокрытых кейсов исчезли из отчёта, и
отчёт стал выглядеть лучше.

Так же молча отвалились ещё три: вторая регулярка в том же файле, `reconcile.py`
в скилле `sync` и `CASE_HEADING` в `uncovered.py`. Первый греп их не поймал,
потому что искал `[A-Z]-[0-9]{2}`, а они написаны как `[A-Z]-\d{2}` — та же
форма, другая запись.

**Правило, которое из этого следует.** Когда меняешь формат id, «линт стал
чище» — это подозрение, а не результат. Сверять надо не число находок, а их
СОСТАВ: после полной починки состав вернулся к исходному
(`12.11×60, 12.17×4, 12.37×3`) при новых id, и вот это и есть доказательство,
что миграция ничего не изменила по смыслу.

**Два места, которые регулярка по id не видит по определению:**

- **Исключения по префиксу.** `prose_required_except_prefix: {acceptance: ['Z']}`
  сравнивалось через `startswith('Z')`, и запрет `CZ-01` перестал быть запретом —
  линт потребовал с него список приёмки, которого у запрета нет по устройству.
  Починено сравнением по букве ВИДА (символ перед дефисом), как уже сделано в
  `rules_ids.py` для буквы роли.
- **Шаблоны, а не id.** `roles[].cases` в `macstack.json` — это `C-*`. Скрипт их
  не трогал, и после миграции линт сообщал «кейсы C-* не принадлежат ни одной
...
## 2026-08-29 — валидация: две копии одного порога уже разошлись, и обе молча (3.8.0)

Прогон `/plugin-creator:validate` по пяти вопросам — чужие имена, хардкод,
консистентность, версии, скрипты. Версии, структура, безопасность и тесты чистые;
нашлись два расхождения, и оба жили ровно в том зазоре, про который в этом файле
уже написано трижды.

**Проблема, первая.** Схема описывает `freshness_days` ВНУТРИ `docRef` — то есть на
документ, и описание прямо говорит «Set it per document». Правило 12.17 и оба хука
читали единственное верхнеуровневое `docs.freshness_days`, которого в схеме не было
вовсе. Итог: настройка, которую обещает документация, не делала ничего, а работала
недокументированная. Форма отказа — худшая из возможных: файл говорит «девяносто
дней», линтер меряет тридцатью, и ни один из них про другого не упоминает.

**Проблема, вторая.** Дату последней сверки линтер поднимал по журналу И по
`archive/reviews/` — для проектов, отревьюенных до переезда вердиктов в журнал. Хук
читал только журнал. На таком проекте линтер молчал, а хук на старте сессии заявлял
«ни разу не сверяли». Это в точности то расхождение, ради предотвращения которого
`macstack_freshness.py` и был написан — его собственный docstring обещает «две копии
одного правила разошлись бы молча», — случившееся внутри самого модуля.

**Fix.** `hooks/macstack_freshness.py` стал единственным владельцем обоих расчётов:
`budget()` (свой срок → общий → 30) и `last_audit()` (журнал + `reviews/` +
`archive/reviews/`). `rules_hygiene.py` импортирует его — путь кладёт `lint_folder.py`
тем же способом, каким уже клал `documents/references`. Верхнеуровневый
`docs.freshness_days` внесён в схему, per-document описание уточнено.

**Root cause.** Не невнимательность, а то, что согласие двух реализаций нигде не
утверждалось — оно держалось на комментарии, просящем его не ломать. Комментарий
исполняется, только когда его читают.

**Проверка.** `tests/test_freshness.py` (13 утверждений) проверяет не «обе считают
одинаково», а что реализация ОДНА: `rules_hygiene._latest_conformance_date is
macstack_freshness.last_audit`. Тест на совпадение поведения пришлось бы дописывать
под каждый новый случай, и он бы отстал. Там же перепись правил: `_load_rule_modules`
ГЛОТАЕТ ошибку импорта и идёт дальше, так что перекрёстный импорт сделал достижимым
путь, на котором линтер отвечает «чисто», потеряв семь правил. Число правил (41)
теперь утверждается, а не подразумевается.

**Severity:** Major (оба).

### Чужие имена — вычищены

- **Имя реального клиента** — 20 вхождений в 8 файлах, включая две пользовательские
  поверхности: пример строки журнала в `intake/SKILL.md` с именем клиента в имени
  PDF и маршрут `/<клиент>/firma` в `format-rules.md`. Запись от 2026-08-27 в этом
  же файле описывает ровно этот класс утечки («Имя пользователя и название
  клиентского проекта — в публичном репозитории») — и всё равно имя осталось ещё в
  семи местах, потому что убирали найденное, а не искали остальное. Замены
  сохраняют измерение («измерено на живом проекте»), теряя только имя.
- **Org с прототипами** была вписана в инструкции как первый шаг выбора софта:
  `gh api "orgs/<org>/repos"` и `"prototype": "github:<org>/<repo>"`. У стороннего
  пользователя публичного плагина там ничего нет, и шаг «prototype first» тихо
  вырождается в пустой список — который читается как «прототипов не существует», а
  это другой и неверный вывод. Теперь скилл спрашивает, где лежат прототипы, и
  честно пропускает шаг без ответа.

### Клиентский пакет больше ничего не грузит со стороны

`package.py` тянул три гарнитуры IBM Plex с CDN. Пакет обещан самодостаточным — его
открывают из почтового вложения и с флешки, — но весомее другое: запрос за шрифтом
сообщает стороннему CDN IP клиента и момент, когда тот открыл документ со своей же
коммерческой спецификацией. Ссылка убрана, IBM Plex остался первым в каждом стеке
(у кого установлен — увидит), фолбэки заменены на системные, НЕСУЩИЕ КИРИЛЛИЦУ:
стек, подобранный по латинице, отвалился бы на первом же русском слове.
**Severity:** Minor.

## 2026-08-29 — reconcile: путь код → документы, и статус задачи как измерение (3.9.0)

**Feature.** Три вещи, которых не было: `/macstack-dev:update` теперь актуализирует и
КЛИЕНТСКИЕ документы, а не только `generated/` и `history/`; появилась команда
`/macstack-dev:reconcile --master=code|docs`, сверяющая весь `macstack/` с деревом кода
в одну ОБЪЯВЛЕННУЮ сторону; статусы задач двигаются по вердиктам аудита во всех
командах, которые называют объём работ.

**Implementation.** Скилл `reconcile` (пять стадий, из которых `--master` читает только
четвёртую), команда `commands/reconcile.md`, скрипт
`skills/planning/references/task_status.py` и `tests/test_task_status.py` (17
утверждений). `update` переписан так, что документную половину он ЗОВЁТ у `reconcile`, а
не несёт свою копию — урок предыдущей записи в этом же файле. `check` и `plan` зовут тот
же скрипт без `--apply`.

**Rationale.** Путь от кода обратно в документы существовал ровно один раз — первый
`/macstack-dev:start`. Дальше код двигался, документы нет, и это ровно тот отказ, ради
предотвращения которого папка существует.

### Гейт остался; исчезла только его ручная часть

Соблазн был снять запрет «не править клиентские документы». Он существует не из
осторожности: в `client/` лежат правки, которые клиент внёс лично, и молчаливая
перезапись такого предложения обесценивает журнал, пакет ревью и строки `handoff`
разом. Поэтому правка код → документ идёт тем же циклом delta → ruling → apply →
journal, что и `intake`, — сменился только источник дельты. Владельцев гейта теперь два
и ровно два: `intake` для того, что прислал клиент, `reconcile` для того, что
оказалось в коде. Правило записано в `documents` (раздел «Who may write into
`client/`»), потому что этот скилл читают первым.

Три вещи останавливают правку в любую сторону, и первая — главная: **statement, на
который клиент ответил** (строка `comment` в журнале). Владелец выбрал именно это
поведение при постановке задачи: спросить, а не перезаписать.

**`--master` обязателен.** Умолчание здесь — это молчаливый выбор победителя в каждом
разногласии папки, а отказ выбирать победителя молча — то единственное, что этот плагин
делает везде.

### Словарь статусов: три места, два словаря, ноль сверок

Найдено при реализации, и это блокировало саму задачу «правильно обновлять статусы».

| Где | Словарь | Кто проверяет |
|---|---|---|
| `fields.status.enum` в контракте | `backlog · todo · in_progress · done · cancelled · …` | правило 12.14 на `TASKS.md` |
| `$defs/taskRef.status` и `milestoneRef.status` схемы | `todo · doing · blocked · done · dropped` | JSON Schema на `macstack.json` |
| `documents.tasks.statuses` | `todo · doing · blocked · done · dropped` | **никто** |

То есть у одной задачи два статуса в двух файлах, записанные разными словарями, и
`macstack.json`, честно отразивший реальный `TASKS.md`, не прошёл бы валидацию схемы.
Карта глифов при этом документировала `doing` и `blocked` — токены, за запись которых
правило 12.14 ставит ошибку: следуя таблице в `planning/SKILL.md`, человек получал
красный линт.

**Fix:** enum схемы расширены до ОБЪЕДИНЕНИЯ (ни один существующий файл не ломается),
ключи карты глифов приведены к тому, что судит 12.14, таблица в `planning/SKILL.md`
исправлена, а `task_status.py` читает словарь из контракта и отказывается писать токен,
которого там нет. Схлопнуть дубли (`doing`/`in_progress`, `cancelled`/`dropped`) —
решение, а не умолчание, и оно за владельцем.

**Заодно:** рабочий пример задачи в `planning/SKILL.md` учил трём вещам, которые тот же
скилл запрещает — `status: doing`, `blocked_by: [A5]` на КЛИЕНТСКОМ вопросе (при
правиле «задача, ждущая ответа клиента, не заводится вовсе») и форму v2 при
`format: v3` в контракте. Переписан.

### Переоткрытие — половина, без которой остальное храповик

`task_status.py` двигает статус в обе стороны. Закрытие по вердикту без переоткрытия
дало бы список работ, который умеет только уменьшаться, а опаснее как раз вторая
сторона: задача стоит `done`, аудит нашёл `absent`, список выглядит короче правды — и
за закрытую задачу никто не заглядывает.

Правило свежести: вердикт применяется, только если он ДАТИРОВАН ПОЗЖЕ самой свежей
собственной даты задачи. Иначе аудит, прогнанный до работы, каждый прогон переоткрывал
бы задачу, закрытую после него, — скрипт спорил бы с человеком, у которого больше
сведений. То же различение, что `reviewed` против `updated` у документов.

**Найдено попутно, не исправлено:** `v3.load_doc(path, lang)` с проектным языком
разбирает документ, переведённый отдельно (`docs.files.<key>.language`), чужой таблицей
меток и возвращает документ БЕЗ ПОЛЕЙ — не ошибку, а ноль сущностей. `task_status.py`
поэтому берёт язык из заголовка самого документа. `uncovered.py` по-прежнему передаёт
проектный: на проекте с переведённым `TASKS.md` он сообщит, что задач нет.
**Severity:** Minor, но молчаливый.
