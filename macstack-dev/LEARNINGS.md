...
`node_modules`, `.git`, `.next` и `dist`. На проекте ohawo в `.claude/worktrees/`
...
проекта разом. Проверено — проект ohawo на старой форме даёт те же 67 находок,
что и до правки.

**Скрипт миграции** — `migrate_ids.py`, и в нём два измеренных урока:

1. **Слепой regex нельзя.** `^[A-Z]-\d{2}$` совпадает и с id ЧУЖОГО документа: в
   ohawo по той же форме записано немецкое ТЗ заказчика (`E-05`, `A-08`, `NF-07`).
